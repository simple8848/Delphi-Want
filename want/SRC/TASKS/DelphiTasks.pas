...
uses
  ...
  WildPaths,
  PatternSets,
  Attributes{$IFDEF Bata},Cndebug{$ENDIF};
...
const
  ...
  BDSRegRoot  = 'Software\Embarcadero\BDS';
  DelphiRegD2007  = 'SOFTWARE\Borland\BDS\5.0';
...
type
  ...
  protected
    ...
    class function IsUseX64:Boolean;
...
//  V := '11';
  ...
  begin
    ...
    {$IFDEF Bata}
    CnDebugger.LogMsg(Format('delphi_ver = %s (%d)',[V,Length(V)]));
    {$ENDIF}
  end;
  if V = '' then begin
     V := '7,11,18,17,16,15,14,13,12,10,9,8,6,5,4';
  end;

  vers := StringToArray(V);
  for i := Low(vers) to High(vers) do
  begin
    ...
    if Path <> '' then
    begin
      Tool := IncludeTrailingPathDelimiter(Path) + ToolName;
...
begin
  ...
  if (fVer<12) then
  begin
    Result := DelphiRegD2007;
    Exit;
  end;
  Result := Format('%s\%s', [BDSRegRoot, version]);
end;
...
class function TCustomDelphiTask.IsUseX64: Boolean;
var
  x64Str  :string;
begin
  Result:=False;
  if (WantUtils.GetEnvironmentVar('delphi_x64', x64Str, False)) and (x64Str<>'') then
    Result := StrToBoolDef(x64Str,False);
end;
...
begin
  ...
  if StrToFloatDef(FVersionFound,0)>=18 then
  begin
    Result := Result + ' -NSSystem.Win;Data.Win;Datasnap.Win;Web.Win;Soap.Win;Xml.Win;Bde;Vcl;Vcl.Imaging;Vcl.Touch;Vcl.Samples;Vcl.Shell;System;Xml;Data;Datasnap;Web;Soap;Winapi;'
  end;
...
begin
  if StrToFloatDef(FVersionFound,0)>11 then
  begin
    if (IsUseX64) then
    begin
      Result := ReadUserOption('Library\Win64', 'Search Path', FVersionFound) + ';' +
                ReadUserOption('Library\Win64', 'SearchPath',  FVersionFound)
    end else
    begin
      Result := ReadUserOption('Library\Win32', 'Search Path', FVersionFound) + ';' +
                ReadUserOption('Library\Win32', 'SearchPath',  FVersionFound)
    end;
  end else
  begin
...
procedure TResourceCompileTask.FindTool;
var
  brcc:string;
begin
  inherited;
  // Modified by Zhongy 2011-12-6 18:15:44 默认使用当前目录下面 brcc
  brcc:=ExtractFilePath(ParamStr(0))+'brcc32.exe';
  if FileExists(brcc) then
    FToolPath:=ToWantPath(brcc);
end;
